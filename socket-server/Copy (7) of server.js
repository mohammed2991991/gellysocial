...
gellybookns.on('connection', socket => {
	...
	socket.on('heartbeat', () => {
	    ...
	    if (!onlineUsers.has(socket.userId)) {
	        ...
	        gellybookns.emit('user.online', { userId: socket.userId });
	    }
	});
    socket.on('join', (room) => {
        socket.join(room);
	...
	socket.on('member.logout', async () => {
	    ...
	    if (token) {
	        try {
	            // تحديث lastSeen في قاعدة البيانات قبل الخروج
	            await axios.post('http://localhost:8000/api/last-seen', {}, {
	                headers: { Authorization: 'Bearer ' + token }
	            });
	            console.log(`LastSeen updated for user ${userId} on logout`);
	        } catch (err) {
	            console.error(`Failed to update lastSeen on logout for user ${userId}:`, err.message);
	            // حتى لو فشل، نكمل عملية الخروج
	        }
	    }

	    // بث آخر ظهور جديد للمستخدمين الآخرين
	    ...
	    // إعلام جميع العملاء بأن المستخدم غير متصل (اختياري)
	    gellybookns.emit('user.offline', { userId, lastSeen: logoutTime });

	    // إعادة بث قائمة المستخدمين المتصلين (بعد إزالته)
	    broadcastOnlineUsers();

	    // فصل socket نهائياً
	    socket.disconnect(true);
	    console.log(`User ${userId} logged out clean with lastSeen updated`);
	});
